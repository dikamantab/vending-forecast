...
class MLService:
    ...
    def preprocess_for_model(self, raw_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Aggregate daily sales per SKU
        daily = df.groupby(["SKU", "date", "Product"]).agg(
        ...
        # Add lag & rolling features (per SKU)
        ...
        # Persist only the last day for each product / sku / category
        daily = daily[daily["date"] == daily["date"].max()]
